...
export const SectionEditor = ({ structuredResume, onChange }: SectionEditorProps) => {
  ...
  const addExperience = () => {
    ...
    if (experienceSection) {
      ...
      const updatedData = [...(experienceSection.content.data as ExperienceBlock[]), newExperience];
  ...
  const updateExperience = (index: number, field: keyof ExperienceBlock, value: any) => {
    const experienceSection = structuredResume.sections.find(s => s.type === 'experience');
    if (experienceSection) {
      const experiences = [...(experienceSection.content.data as ExperienceBlock[])];
    ...
    <div className="space-y-6">
      ...
      {structuredResume.sections.map((section) => {
        ...
        if (section.type === 'experience') {
          return (
            <div key={section.id}>
              {renderExperienceEditor(section.content.data as ExperienceBlock[])}
            </div>
          );
        }
