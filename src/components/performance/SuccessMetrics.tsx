...
import { RefreshCw, Building, Calendar, Clock } from "lucide-react";
...
export const SuccessMetrics = ({ metrics, applications, onRefreshMetrics }: SuccessMetricsProps) => {
  ...
  const formatStatusDate = (updatedAt: string, dateApplied: string) => {
    const updatedDate = new Date(updatedAt);
    const appliedDate = new Date(dateApplied);
    const today = new Date();
    
    // Check if status was updated today
    const isToday = updatedDate.toDateString() === today.toDateString();
    
    // Check if updated_at is same as when applied (meaning no status change)
    const isOriginalStatus = Math.abs(updatedDate.getTime() - appliedDate.getTime()) < 60000; // within 1 minute
    
    if (isOriginalStatus) {
      return {
        displayDate: appliedDate.toLocaleDateString('en-US', {
          month: 'short',
          day: 'numeric'
        }),
        fullDate: `Applied: ${appliedDate.toLocaleDateString('en-US', {
          month: 'short',
          day: 'numeric',
          year: 'numeric'
        })}`,
    ...
    if (isToday) {
      return {
        displayDate: updatedDate.toLocaleTimeString('en-US', {
          hour: 'numeric',
          minute: '2-digit',
          hour12: true
        }),
        fullDate: `Status updated: ${updatedDate.toLocaleDateString('en-US', {
          month: 'short',
          day: 'numeric',
          year: 'numeric'
          ...
          minute: '2-digit',
          hour12: true
        })} | Applied: ${appliedDate.toLocaleDateString('en-US', {
          month: 'short',
          day: 'numeric',
          year: 'numeric'
        })}`,
    ...
    return {
      displayDate: updatedDate.toLocaleDateString('en-US', {
        month: 'short',
        day: 'numeric'
      }),
      fullDate: `Status updated: ${updatedDate.toLocaleDateString('en-US', {
                  ...
                  .map(([status, apps]) => (
                    <div key={status} className="space-y-2">
                      ...
                      <div className="ml-2 space-y-1">
                        {apps.map((app) => {
                          const dateInfo = formatStatusDate(app.updated_at || app.created_at, app.date_applied);
                          return (
                            <div 
                              key={app.id} 
                            ...
                            >
                              ...
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <div className="flex items-center gap-1 text-xs text-muted-foreground flex-shrink-0 cursor-help">
                                    {dateInfo.isStatusUpdate ? (
                                ...
                                <TooltipContent>
                                  <p className="text-xs">{dateInfo.fullDate}</p>
                                </TooltipContent>
                              </Tooltip>
                            </div>
                          );
