      ...
      <CardContent className="p-4">
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            {sections.map((section, index) => <button key={section.id} onClick={() => onSectionSelect(index)} className={`w-full p-3 rounded-lg border transition-all text-left overflow-hidden ${getStatusColor(section, index)}`}>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1 flex-wrap">
                        ...
                        {section.is_required && <Badge variant="destructive" className="text-xs h-5 shrink-0">
                            Required
                          </Badge>}
                      </div>
                    </div>
                  ...
                  {index === currentSection && <div className="text-primary text-sm font-medium shrink-0 ml-2">
                      Current
                    </div>}
                </div>
              </button>)}
          ...
          {/* Completion Status */}
          {progressPercentage === 100 && <div className="p-3 bg-green-50 dark:bg-green-950/20 rounded-lg border border-green-200 dark:border-green-800">
